...
const Unclaimed = ({ totalRequest, unclaimedRequests, CountUp }) => {
  ...
  useEffect(() => {
    if (totalRequest === 0) {
      ...
      const percent = (unclaimedRequests.length / totalRequest) * 100;
      setPercentage(percent);
    }
  }, [unclaimedRequests, totalRequest]);
        ...
        <div className="card-hover shadow-sm rounded p-3 h-100 bg-white">
          ...
          <div className="text-start mt-2 d-flex justify-content-start align-items-center gap-2">
            <h4 className="text-secondary m-0">
              <CountUp end={unclaimedRequests.length} duration={1.5} />
            </h4>
          </div>
        </div>
      </button>
        ...
        >
          <Modal.Title>
            <h5 className="m-0 text-white">
              Unclamied ({unclaimedRequests.length})
            </h5>
          </Modal.Title>
        </Modal.Header>
        <RequestTableTemplate Requests={unclaimedRequests} />
        <Modal.Footer>
          <Button
            variant="secondary"
            className="border-0"
